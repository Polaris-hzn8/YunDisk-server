/*
 ...
 filename: dealfile_cgi.c
 Update Time: Wed 16 Aug 2023 17:30:02 CST
 brief: 分享删除文件、文件pv字段处理
*/
...
int share_file(char* user, char* md5, char* filename)
{
  ...
  */
    ...
    { //===2、如果集合没有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
        ...
        sprintf(
            sql_cmd,
            "select * from share_file_list where md5 = '%s' and filename = '%s'",
            md5, filename);

        //返回值： 0成功并保存记录集，1没有记录集，2有记录集但是没有保存，-1失败
        ret2 = process_result_one(conn, sql_cmd,
            NULL); //执行sql语句, 最后一个参数为NULL
...
int del_file(char* user, char* md5, char* filename)
{
  ...
  */
    ...
    { //===2、如果集合没有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
        ...
        //查看该文件是否已经分享了
        sprintf(sql_cmd,
            "select shared_status from user_file_list where user = '%s' and "
            "md5 = '%s' and filename = '%s'",
            user, md5, filename);
...
int pv_file(char* user, char* md5, char* filename)
{
    ...
    //查看该文件的pv字段
    sprintf(sql_cmd,
        "select pv from user_file_list where user = '%s' and md5 = '%s' and "
        "filename = '%s'",
        user, md5, filename);
    ...
    //更新该文件pv字段，+1
    sprintf(sql_cmd,
        "update user_file_list set pv = %d where user = '%s' and md5 = '%s' "
        "and filename = '%s'",
